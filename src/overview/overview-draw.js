 ...
 */
 const drawLegends = (legends, legendHeight) => {
  ...
  for (let i = 0; i < 2; i++){
    ...
    let range2 = cnnLayerRanges.local[start + 3];
    ...
    let localLegendScale2 = d3.scaleLinear()
      .range([0, 4 * nodeLength + 3 * hSpaceAroundGap - 1.2])
    ...
    let localLegend2 = legends.append('g')
      ...
      .classed('hidden', !detailedMode || selectedScaleLevel !== 'local')
      .attr('transform', `translate(${nodeCoordinate[start + 3][0].x}, ${0})`);
    ...
    localLegend2.append('rect')
      .attr('width', 4 * nodeLength + 3 * hSpaceAroundGap)
      .attr('height', legendHeight)
      .style('fill', 'url(#convGradient)');
  }
  ...
  for (let i = 0; i < 2; i++){
    ...
    let moduleLegendScale = d3.scaleLinear()
      .range([0, 7 * nodeLength + 5 * hSpaceAroundGap +
    ...
    moduleLegend.append('rect')
      .attr('width', 7 * nodeLength + 5 * hSpaceAroundGap +
  ...
  let globalLegendScale = d3.scaleLinear()
    .range([0, 14 * nodeLength + 10 * hSpaceAroundGap +
  ...
  globalLegend.append('rect')
    .attr('width', 14 * nodeLength + 10 * hSpaceAroundGap +
  ...
  let outputLegend = legends.append('g')
    ...
    .attr('transform', `translate(${nodeCoordinate[15][0].x}, ${0})`);
...
export const drawCNN = (width, height, cnnGroup, nodeMouseOverHandler,
  ...
  hSpaceAroundGap = (width - nodeLength * numLayers) / (12 + 5 * gapRatio);
  ...
  let detailedLabels = svg.selectAll('g.layer-detailed-label')
    ...
    .on('click', (d) => {
      ...
      if (d.name.includes('batch')) {target = 'batch normalization'}
  ...
  detailedLabels.append('text')
    ...
    .text(d => {
      if (d.name.includes('batch_normalization')) {
        let shortName = d.name.replace('batch_normalization', 'bn')
  ...
  labels.append('text')
    ...
    .text(d => {
      ...
      if (d.name.includes('batch')) {return 'batch normalization'}
...
export const updateCNN = () => {
  ...
  for (let i = 0; i < 2; i++){
    ...
    let localLegendScale2 = d3.scaleLinear()
      .range([0, 4 * nodeLength + 3 * hSpaceAroundGap])
      .domain([-range2 / 2, range2 / 2]);
...
export const updateCNNLayerRanges = () => {
  ...
  let numOfComponent = (numLayers - 2) / 7;
  for (let i = 0; i < numOfComponent; i++) {
    let curArray = cnnLayerRangesLocal.slice(1 + 7 * i, 1 + 7 * i + 7);
