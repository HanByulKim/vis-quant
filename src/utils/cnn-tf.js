...
class BNNode {
  constructor(layerName, index, type, gamma, beta, run_avg, run_var, output) {
    this.layerName = layerName;
    this.index = index;
    this.type = type;
    this.gamma = gamma;
    this.beta = beta;
    this.run_avg = run_avg;
 ...
 */
const constructCNNFromOutputs = (allOutputs, model, inputImageTensor, selectedShift) => {
  let cnn = [];
  ...
  for (let l = 0; l < model.layers.length; l++) {
    ...
    } else if (layer.name.includes('batch_normalization')) {
      curLayerType = nodeType.BN;
    ...
    switch (curLayerType) {
      ...
      case nodeType.BN: {
        //console.log(layer)
        let gammas = layer.gamma.val.arraySync();
        let betas = layer.beta.val.arraySync();
        let run_avgs = layer.movingMean.val.arraySync();
        let run_vars = layer.movingVariance.val.arraySync();

        for (let i = 0; i < outputs.length; i++) {
          let node = new BNNode(layer.name, i, curLayerType, gammas[i], betas[i]*selectedShift, run_avgs[i]*selectedShift, run_vars[i], outputs[i]);
          //console.log(layer.name, gammas[i], betas[i], outputs[i])
 ...
 * @param {Model} model Loaded tf.js model.
 */
export const constructCNN = async (inputImageFile, model, selectedShift) => {
  // Load the image file
  console.log("constructCNN")
  ...
  for (let l = 0; l < model.layers.length; l++) {
    console.log("layers", model.layers[l])
    if (model.layers[l].name.includes('batch_normalization')){
      const newval = model.layers[l].beta.val.arraySync();
      for (let j = 0; j < newval.length; j++){ newval[j] = newval[j]*selectedShift[0]; }
      model.layers[l].beta.val.assign(tf.tensor(newval));
      const newval2 = model.layers[l].movingMean.val.arraySync();
      for (let j = 0; j < newval2.length; j++){ newval2[j] = newval2[j]*selectedShift[0]; }
      model.layers[l].movingMean.val.assign(tf.tensor(newval2));
      console.log("movingMean", model.layers[l].movingMean.val.arraySync());
    }
    if (model.layers[l].name.includes('batch_normalization_1')){
      const newval = model.layers[l].beta.val.arraySync();
      for (let j = 0; j < newval.length; j++){ newval[j] = newval[j]*selectedShift[1]; }
      model.layers[l].beta.val.assign(tf.tensor(newval));
      const newval2 = model.layers[l].movingMean.val.arraySync();
      for (let j = 0; j < newval2.length; j++){ newval2[j] = newval2[j]*selectedShift[1]; }
      model.layers[l].movingMean.val.assign(tf.tensor(newval2));
    }
    if (model.layers[l].name.includes('batch_normalization_2')){
      const newval = model.layers[l].beta.val.arraySync();
      for (let j = 0; j < newval.length; j++){ newval[j] = newval[j]*selectedShift[2]; }
      model.layers[l].beta.val.assign(tf.tensor(newval));
      const newval2 = model.layers[l].movingMean.val.arraySync();
      for (let j = 0; j < newval2.length; j++){ newval2[j] = newval2[j]*selectedShift[2]; }
      model.layers[l].movingMean.val.assign(tf.tensor(newval2));
    }
    if (model.layers[l].name.includes('batch_normalization_3')){
      const newval = model.layers[l].beta.val.arraySync();
      for (let j = 0; j < newval.length; j++){ newval[j] = newval[j]*selectedShift[3]; }
      model.layers[l].beta.val.assign(tf.tensor(newval));
      const newval2 = model.layers[l].movingMean.val.arraySync();
      for (let j = 0; j < newval2.length; j++){ newval2[j] = newval2[j]*selectedShift[3]; }
      model.layers[l].movingMean.val.assign(tf.tensor(newval2));
    }
  ...
  let cnn = constructCNNFromOutputs(outputs, model, inputImageTensor, selectedShift);
  return cnn;
}
