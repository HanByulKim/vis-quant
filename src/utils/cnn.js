...
const constructNNFromJSON = (nnJSON, inputImageArray) => {
  ...
  nnJSON.forEach(layer => {
    ...
    for (let i = 0; i < layer.num_neurons; i++) {
      ...
      } else if (curLayerType == nodeType.BN || curLayerType === nodeType.RELU || curLayerType === nodeType.POOL) {
...
const singleBatchNorm = (input, gamma, beta, run_avg, run_var) => {
  console.assert(input.length === input[0].length, 'BN input is not square');

  let result = init2DArray(input.length, input.length, 0);

  for (let i = 0; i < input.length; i++) {
    for (let j = 0; j < input.length; j++) {
      result[i][j] = gamma * (input[i][j] - run_avg) / Math.sqrt(run_var + 0.001) + beta
    }
  }
  return result;
}

const batchNorm = (curLayer) => {
  console.assert(curLayer[0].type === 'batchnorm', 'Wrong layer type');

  for (let i = 0; i < curLayer.length; i++) {
    let curNode = curLayer[i];
    let preNode = curNode.inputLinks[0].source;
    curNode.output = singleBatchNorm(preNode.output, curNode.gamma, curNode.beta, curNode.run_avg, curNode.run_var);
  }
}
